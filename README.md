...
مدل‌های زبانی همی‌شه دقیقاً همان چیزی را که انتظار داریم برنمی‌گردانند.
...
توی Settings کلید «Mock mode» رو روشن کنید — کل pipeline (glossary، ترجمهی کل کتاب، QA، finalize و دانلود) روی یه fake داخلی اجرا می‌شه: صفر درخواست API، صفر هزینه، بدون نیاز به کلید. همه‌ی خروجی‌ها واضح علامت‌گذاری می‌شن (【آزمایشی】) که هیچ‌وقت با ترجمهی واقعی اشتباه گرفته ن‌شن. توی صفحه‌ی آپلود هم دکمه‌ی «کتاب نمونه» هست برای تست یک‌کلیکی.
...
* ترجمه‌ی چند‌زبانه فراتر از EN → FA
* User accounts
* Multi-tenancy
* پرداخت
* Bulk editing
